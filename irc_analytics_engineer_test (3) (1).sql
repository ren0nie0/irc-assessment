...
*/
SELECT account_id,close_date, SUM(amount) as total_amount
                    FROM donations 
                    WHERE account_id in (
                                         SELECT account_id
                                             FROM accounts
...
with numbered_donations as
                ...
                SELECT billing_country, ROUND(AVG(amount),2)::money from first_donations GROUP BY 1''')
...
/*
  ...
  I believe the readability of the query could be greatly enhanced by using proper indents uniformly. 
  I also think breaking this up into multiple queries would increase modularity and greatly save system resources being used by the numerous joins.
